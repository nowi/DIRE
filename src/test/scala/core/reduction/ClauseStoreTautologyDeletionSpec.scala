...
class ClauseStoreTautologyDeletionSpec extends Spec with ShouldMatchers with Logging {

  // override in specific tests
  val clauseStoreTautologyDeleter: ClauseStoreTautologyDeletion = ClauseStoreTautologyDeletor
  ...
  describe("A object implementing the TautologyDeletion Trait") {
    it("should delete tautologies from clause store") {
      ...
      val clauseStore = List(StandardClause(Negation(Predicate("American", x)), Predicate("American", x),
        Negation(Predicate("Sells", x, y, z)), Negation(Predicate("Hostile", z)), Predicate("Criminal", x)))

      clauseStoreTautologyDeleter(clauseStore) should equal(Nil)
    ...
    it("should delete tautologies from clause store but not all clauses") {
      ...
      clauseStoreTautologyDeleter(clauseStore) should equal(List(StandardClause(Negation(Predicate("American", x)),
        Negation(Predicate("Sells", x, y, z)), Negation(Predicate("Hostile", z)), Predicate("Criminal", x))
        ))
