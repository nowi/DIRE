...
//
//    if (recordProofSteps) {
//      // TODO checkc If this this will loose history
//      // register all clauses in record
//      //backgroundClauses.foreach(inferenceRecorder.recordClause(_, None, None))
//      cleaned.foreach(inferenceRecorder.recordRecievedClause(_, None, None))
//
//    }
//
//
...
//
//              case Nil => {
//                // there has been nothing resolved
//                log.trace("There has been nothing resolved for headClause : %s", headClause)
//              }
//
//              case resolutions => {
//                for (resolution <- resolutions) {
//                  resolution match {
//                    case core.resolution.SuccessfullResolution(resolvedClauses, parent1, parent2) => {
//                      val derivedClauses = CNFClauseStore(resolvedClauses) // TODO optimize here , because this is
//                      // always only a singleton clause
//
...
//
//                                notifyObservers(SuccessfullResolution(unique, parent1, parent2))
//                                if (recordProofSteps) {
//                                  // record
//                                  for (derived <- unique) {
//
//                                    inferenceRecorder.recordClause(derived, Some(parent1), Some(parent2))
//                                  }
//
//
//                                  log.info(formatClauseStore(unique, true))
//                                }
//
//                              }
//
...
//                                case true => sub(workedOff, unique)
...
//                      error("Resolver only delivers Successfull resolutions or nil , cannot be here !")
//                    }
//                  }
//
//                }
...
//  }
//
//  def resolve(a: FOLClause, b: ClauseStorage): Iterable[ResolutionResult] = {
//    b match {
